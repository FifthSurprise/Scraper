...
public class JobScraperGUI extends Application {
	...
	public void start(Stage stage) throws Exception {
		...
		companyNameCol.setOnEditCommit(
			    new EventHandler<CellEditEvent<ObsCompany, String>>() {
			        @Override
			        public void handle(CellEditEvent<ObsCompany, String> t) {
			            ((ObsCompany) t.getTableView().getItems().get(
		...
		urlNameCol.setCellValueFactory(new PropertyValueFactory<ObsCompany, String>("jobUrl"));
		...
		linkCol.setCellValueFactory(new Callback<CellDataFeatures<ObsCompany, ObsCompany>, ObservableValue<ObsCompany>>() {
            @Override public ObservableValue<ObsCompany> call(CellDataFeatures<ObsCompany, ObsCompany> features) {
		...
		linkCol.setCellFactory(new Callback<TableColumn<ObsCompany, ObsCompany>, TableCell<ObsCompany, ObsCompany>>() {
          @Override public TableCell<ObsCompany, ObsCompany> call(TableColumn<ObsCompany, ObsCompany> linkCol) {
            return new TableCell<ObsCompany, ObsCompany>() {
              final Button button = new Button("Link"); 
              @Override public void updateItem(final ObsCompany obs, boolean empty) {
		...
		table.setItems(data);
		table.getColumns().addAll(companyNameCol, linkCol, urlNameCol,notesCol);
		...
		vbox.getChildren().addAll(label, table, save,delete);
